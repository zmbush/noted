...
export default function notes(
  ...
  state = initialState,
  action:
    | { type: NotedEvent.NotesFetched; notes: NoteWithTags[] }
    | { type: NotedEvent.NotesUpdateNote; note: NoteWithTags }
    | { type: NotedEvent.NotesDeleteNote; id: number }
    | { type: NotedEvent.ApiError; error: ErrorData }
    | { type: NotedEvent.UserSignedOut }
    | {},
): State {
  if ('type' in action) {
    switch (action.type) {
